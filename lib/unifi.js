...
export class UnifiError extends Error {
    constructor(message, {status, path, body, retryAfter} = {}) {
        super(message);
        this.name = 'UnifiError';
        this.status = status;
        this.path = path;
        this.body = body;
        /** ms the controller asked us to wait (Retry-After), or null */
 ...
 * 429 `AUTHENTICATION_FAILED_LIMIT_REACHED` (too many failed logins, the account is locked for a
 * while); legacy: 400 `{meta: {rc: "error", msg: "api.err.Invalid"}}`, `api.err.Ubic2faTokenRequired`.
 */
export function loginFailure(status, parsed) {
    ...
    if (status === 429) {
        return `rate-limited by the controller${code ? ` (${code})` : ''} — too many login attempts, the account may be locked for a while`;
    }
    if (status !== 200) {
        return `http ${status}`;
    }
...
export class UnifiController {
    ...
    async doLogin() {
        ...
        if (failure) {
            throw new UnifiError(`login failed: ${failure}`, {
                status: res.status,
                path,
                body: res.body,
